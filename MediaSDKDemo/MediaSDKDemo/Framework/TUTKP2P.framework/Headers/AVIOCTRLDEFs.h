...
typedef enum 
{
    ...
    IOTYPE_USER_IPCAM_CALL_REQ            = 0x0901,    // start oneway call request
    IOTYPE_USER_IPCAM_CALL_RESP           = 0x0902,    // start oneway call response
    
    IOTYPE_USER_IPCAM_IDINFO                    = 0x0905,    // send IDs Info
...
typedef struct
{
    unsigned char myID[6];        // my ID
    unsigned char myUID[20];      // my UID
    unsigned char callType;        //  1,oneway; 2, twoway
    unsigned char beInvited;
    unsigned char infoCount;       // quantity of  account
    unsigned char reserved[3];
    AccountInfo   info[4];         // max number of accounts 4
}SMsgAVIoctrlCallReq;
...
/* IOTYPE_USER_IPCAM_IDINFO= 0x0905;
   */
typedef struct
{
    unsigned int count;
