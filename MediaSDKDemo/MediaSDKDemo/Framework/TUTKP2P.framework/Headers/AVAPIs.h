 ...
 * \brief Start an AV server
 *
 * \details Start a new re-send protocal supported AV server with user defined authentication function.
 *			Any AV client wanting to connect with this AV server shall
 *			pass the authentication with view account and password. Whether the re-send mechanism
 *          is enabled or not depends on AV client settings and will set the result into
 *          pnResend parameter.
 ...
 * \param pnResend [out] Set the re-send is enabled or not.
 *
 * \param pnNewClient [out] Set to 1 if the connected AV client support new re-send protocal.
 *
 * \return AV channel ID if return value >= 0
 * \return Error code if return value < 0
 *			- #AV_ER_INVALID_ARG The IOTC session ID is incorrect or reuse of IOTC session ID
 ...
 *				please check if the IOTC channel is used correctly
 *
 * \see avServStart(), avServStart2(), avServStart3(), avServExit(), avServStop()
 *
 * \attention (1) This function is a block process.<br><br>
 *			  (2) The IOTC channel of specified channel ID will be turned on automatically
 *			 	   by avNewServStart()
 *
 */
AVAPI_API int avNewServStart(int nIOTCSessionID, authFn pfxAuthFn, unsigned int nTimeout, unsigned int nServType,
                             unsigned char nIOTCChannelID, int *pnResend, int *pnNewClient);


/**
 * \brief Start an AV client
 *
 * \details Start a new re-send protocal supported AV client by providing view account and password.
 *			It shall pass the authentication of the AV server before receiving
 *			AV data. Whether the re-send mechanism is enabled or not depends on AV server settings
 *          and will set the result into pnResend parameter.
 *
 ...
 * \param pnResend [out] The re-send is enabled or not.
 *
 * \param pnNewServer [out] Set to 1 if the connected AV server support new re-send protocal.
 *
 * \return AV channel ID if return value >= 0
 * \return Error code if return value < 0
 *			- #AV_ER_INVALID_ARG The IOTC session ID is incorrect or reuse of IOTC session ID
 ...
 * \attention (1) This function is a block process.<br><br>
 *			  (2) The IOTC channel of specified channel ID will be turned on automatically
 *				  by avNewClientStart().<br><br>
 *
 */
AVAPI_API int avNewClientStart(int nIOTCSessionID, const char *cszViewAccount, const char *cszViewPassword, unsigned int nTimeout,
                               unsigned int *pnServType, unsigned char nIOTCChannelID, int *pnResend, int *pnNewServer);
 ...
 *
 */
AVAPI_API int AV_Status_Check(int nAVChannelID, struct st_AvStatus *psAvStatus);
